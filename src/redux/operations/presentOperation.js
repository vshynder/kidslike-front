...
const addPresent = (fD) => (dispatch,getState) => {
  ...
  const {
    ...
    console.dir(fD);
    axios.post('https://kidslike-back-end.herokuapp.com/api/presents/' ,fD)
    .then(res => {
...
console.log('отправляю idPresent',idPresent);
    axios.patch(`https://kidslike-back-end.herokuapp.com/api/presents/${idPresent}`, fD) ///  заглушка на end point 
        .then(res => {
          console.log(res);
...
const removePresent = id => (dispatch,getState) => {
  ...
  setAuthToken(acToken);
    ...
    axios.delete(`https://kidslike-back-end.herokuapp.com/api/presents/${id}`)// временна заглушка .. передаем id present для удаления 
...
const getAllPresents =  () => (dispatch,getState) => {
  ...
  if (!acToken) {
    ...
    axios.get(`https://kidslike-back-end.herokuapp.com/api/presents`)// временна заглушка .. передаем id present для удаления 
    .then((res)=>{
